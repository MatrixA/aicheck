...
/// PNG text chunk keywords that may contain AI tool references.
...
/// Detect AI signals from PNG text chunks (tEXt, iTXt).
            ...
            b"iTXt" => {
                ...
                if let Some(null_pos) = chunk_data.iter().position(|&b| b == 0) {
                    if let Ok(keyword) = std::str::from_utf8(&chunk_data[..null_pos]) {
                        ...
                        // Find text after 3 more null separators (or parse what we can)
...
fn check_text_chunk(keyword: &str, value: &str, signals: &mut Vec<Signal>) {
    ...
    if let Some(tool_name) = known_tools::match_ai_tool(value) {
        signals.push(Signal {
            source: SignalSource::PngText,
            confidence: Confidence::Low,
            description: format!("PNG {} references AI tool", keyword),
            tool: Some(tool_name.to_string()),
            details: vec![(keyword.to_string(), truncate(value, 200))],
        });
    }
}
